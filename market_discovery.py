...
retry=Retry(total=3, connect=3, read=3, backoff_factor=0.4, status_forcelist=[429,500,502,503,504], allowed_methods=frozenset(['GET']))
...
def _ts(v):
    ...
    try: return datetime.fromisoformat(s).astimezone(timezone.utc).timestamp()
...
def _normalize(m):
    ...
    mm=re.search(r'^(btc|eth|sol|bnb)-updown-5m-(\d{9,12})$',slug.lower())
    if not mm: return None
    asset=mm.group(1).upper(); start=float(mm.group(2)); end=start+300
    tokens=_parse(m.get('clobTokenIds') or m.get('clob_token_ids'))
    outcomes=_parse(m.get('outcomes'))
    if len(tokens)<2: return None
    mapping={str(o).strip().lower():str(t) for o,t in zip(outcomes,tokens)}
    up=mapping.get('up'); down=mapping.get('down')
    ...
    return {
        'id':str(m.get('id') or condition),'condition':condition,
        'market':str(m.get('question') or m.get('title') or slug),'slug':slug,'asset':asset,
        'up':up,'down':down,'start_ts':start,'end_ts':end,'raw':m,
...
def _get_market_by_slug(slug):
    # Official Gamma market-by-slug route. Query-param lookup is retained as a fallback.
...
def discover(now=None, lookahead=600):
    now=time.time() if now is None else float(now); base=int(now//300)*300; out={}
    diagnostics=[]
    for asset,prefix in ASSET_SLUGS.items():
        ...
        for start in (base-300,base,base+300,base+600):
            ...
            try: raw=_get_market_by_slug(slug)
                ...
                diagnostics.append(f'{asset}:{slug}:ERROR:{type(e).__name__}:{e}'); continue
    ...
    # Global fallback is paginated; it is a recovery path, not the primary path.
    if not out:
        offset=0
        while offset<2000:
            try:
                r=SESSION.get(f'{GAMMA}/markets',params={'active':'true','closed':'false','limit':500,'offset':offset,'order':'endDate','ascending':'true'},timeout=15)
                r.raise_for_status(); rows=_rows(r)
            except Exception as e:
                diagnostics.append(f'GLOBAL_FALLBACK:ERROR:{type(e).__name__}:{e}'); break
            if not rows: break
            for raw in rows:
                x=_normalize(raw)
            ...
            offset+=500
    if not out:
        LOG.warning('DISCOVERY | discovered=0 | %s', ' | '.join(diagnostics))
    else:
        LOG.info('DISCOVERY | discovered=%d | assets=%s | %s',len(out),','.join(sorted({x['asset'] for x in out.values()})),' | '.join(diagnostics))
...
def book(token):
    ...
    def levels(rows, side):
        ...
        for x in rows or []:
            try:
                price=float(x.get('price') if isinstance(x,dict) else x[0]); size=float(x.get('size') if isinstance(x,dict) else x[1])
    ...
    bid=levels(d.get('bids'),'bid'); ask=levels(d.get('asks'),'ask')
    return (bid[0] if bid else None),(ask[0] if ask else None),(bid[1] if bid else 0.0),(ask[1] if ask else 0.0)

def resolve(market):
    ident=market.get('id') if isinstance(market,dict) else str(market)
    slug=market.get('slug') if isinstance(market,dict) else None
    raw=None
    if ident:
    ...
    if bool(raw.get('closed')) or bool(raw.get('resolved')):
        return None,None,'CLOSED_UNRESOLVED'
...
def _winner(m):
    ...
    if len(tokens)==len(outcomes)==len(prices) and tokens:
        ...
        for i,v in enumerate(vals):
            if v>=0.999999: return str(tokens[i]),str(outcomes[i])
    return None,None
