    ...
    while True:
        try:
            ...
            for m in list(markets.values()):
                ...
                meta={"slug":m["slug"],"asset":m["asset"],"start_ts":m["start_ts"],"end_ts":m["end_ts"],"market_id":m["id"],"up_token":m["up"],"down_token":m["down"],"model_version":"V12_TRADER_BEHAVIOR_COPY","entry_count_before":ec,"seconds_since_first_entry":sfirst,"regime":strategy.fine_band(sig.price)[1], "fine_band":strategy.fine_band(sig.price)[0],"execution_mode":"PASSIVE_BID_PROXY","target_capital":strategy.desired_capital(sig.price),"bid_size":bid_size}
                ...
                p(f"TRADE PAPER | V12 TRADER COPY | asset={m['asset']} | side={sig.side} | notional=${notion:.2f} | bid=${sig.price:.4f} | target=${meta['target_capital']:.2f} | entry_count={ec} | {sig.reason}")
